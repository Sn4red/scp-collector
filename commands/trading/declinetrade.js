...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (tradeDocument.recipient !== userId) {
            await interaction.editReply(`${process.env.EMOJI_ERROR}  Error. You cannot decline this trade because it wasn't sent it to you.`);
            return;
        }
        ...
        const reply = await interaction.editReply({
            content: `${process.env.EMOJI_STOP}  Are you sure you want to decline the trade request **\`${tradeSnapshot.id}\`** from \`${issuerNickname}\`?`,
        ...
        collector.on('collect', async (button) => {
            if (button.customId === 'confirm') {
                ...
                try {
                    ...
                    await interaction.followUp({ content: `${process.env.EMOJI_CHECK}  Trade >> **\`${tradeSnapshot.id}\`** << was declined. ${process.env.EMOJI_TRASH}`, ephemeral: true });
                    ...
                    if (error.message.includes(errorMessage1) || error.message.includes(errorMessage2)) {
                        ...
                        await interaction.followUp({ content: `${process.env.EMOJI_ERROR}  An error has occurred while trying to decline the request. Please try again.`, ephemeral: true });
                    }
                }
            }
