...
module.exports = {
    ...
    async execute(interaction) {
        ...
        interaction.awaitModalSubmit({
            ...
            const recipientValue = modalInteraction.fields
                .getTextInputValue('txtRecipient');
            const issuerCardValue = modalInteraction.fields
                .getTextInputValue('txtIssuerCard');
            const issuerHolographicValue = modalInteraction.fields
                .getTextInputValue('txtIssuerHolographic');
            const recipientCardValue = modalInteraction.fields
                .getTextInputValue('txtRecipientCard');
            const recipientHolographicValue = modalInteraction.fields
                .getTextInputValue('txtRecipientHolographic');

            const fieldsValidation = validateFields(
                recipientValue,
                issuerCardValue,
                issuerHolographicValue,
                recipientCardValue,
                recipientHolographicValue,
            );
            ...
            if (fieldsValidation.errorState) {
             ...
             const specialConditionValidations = validateSpecialConditions(
                userId,
                fieldsValidation.recipientValue,
                fieldsValidation.fixedIssuerCardValue,
                fieldsValidation.fixedIssuerHolographicValue,
                fieldsValidation.fixedRecipientCardValue,
                fieldsValidation.fixedRecipientHolographicValue,
            );

             // ! If the user is trying to create a trade request with himself
             // ! or trading the same card, returns an error message.
            if (specialConditionValidations.errorState) {
                const errorMessage = new TextDisplayBuilder()
                    .setContent(specialConditionValidations.errorMessage);
            ...
            const errorMessage1 =
                'The user you are trying to trade with is either not ' +
                    'registered or not found.';
            const errorMessage2 =
                'The user you are trying to trade with has disabled trade ' +
                    'requests.';
            const errorMessage3 =
                'It seems that you don\'t have the card you are offering.';
            const errorMessage4 =
                'It seems that the user doesn\'t have the card you want.';

            try {
                await database.runTransaction(async (transaction) => {

                    const recipientUserReference = database.collection('user')
                        .doc(recipientValue);
                    const recipientUserSnapshot = await recipientUserReference
                        .get();

                    // ! If the recipient is not registered or not found,
                    ...
                    // ! If the recipient has disabled trade requests, returns
                    // ! an error message.
                    ...
                    // ! If the issuer doesn't have the card, returns an error
                    // ! message.
                    ...
                    // ! If the recipient doesn't have the card, returns an
                    // ! error message.
                ...
                const message = new TextDisplayBuilder()
                    .setContent(
                        `${process.env.EMOJI_CHECK}  Trade request sent ` +
                            `with ID **\`${tradeEntry.id}\`**. You can use ` +
                            '/`viewtrade` to manage the request.',
                    );
                ...
                if (error.message.includes(errorMessage1) ||
                    ...
                    const errorMessage = new TextDisplayBuilder()
                        .setContent(`${process.env.EMOJI_ERROR}  Request ` +
                    ...
                    const errorMessage = new TextDisplayBuilder()
                        .setContent(
                            `${process.env.EMOJI_ERROR}  An error has ` +
                                'occurred while trying to create the trade ' +
            ...
            console.log(
                `${new Date()} >>> *** WARNING: trade.js *** by ${userId} ` +
            ...
            const errorMessage = new TextDisplayBuilder()
                .setContent(
                    `${process.env.EMOJI_ERROR}  Request cancelled due to ` +
                        'inactivity.',
                );
...
function displayModal(userId) {
    ...
    const recipientRow = new ActionRowBuilder()
        .addComponents(txtRecipient);
    const recipientCardRow = new ActionRowBuilder()
        .addComponents(txtRecipientCard);
    const recipientHolographicRow = new ActionRowBuilder()
        .addComponents(txtRecipientHolographic);
    const issuerCardRow = new ActionRowBuilder()
        .addComponents(txtIssuerCard);
    const issuerHolographicRow = new ActionRowBuilder()
        .addComponents(txtIssuerHolographic);
        
    modal.addComponents(
        recipientRow,
        recipientCardRow,
        recipientHolographicRow,
        issuerCardRow,
        issuerHolographicRow,
    );
...
function validateFields(
    ...
    // * The validation is performed to ensure that the user ID contains only
    // * numbers.
    ...
    const recipientCardValidation = /^scp-\d{3,4}$/i
        .test(fixedRecipientCardValue);
    ...
    const fixedIssuerHolographicValue = issuerHolographicValue.charAt(0)
        .toUpperCase() + issuerHolographicValue.toLowerCase().slice(1);
    const fixedRecipientHolographicValue = recipientHolographicValue.charAt(0)
        .toUpperCase() + recipientHolographicValue.toLowerCase().slice(1);

    const issuerHolographicValidation = validHolographics
        .includes(fixedIssuerHolographicValue);
    const recipientHolographicValidation = validHolographics
        .includes(fixedRecipientHolographicValue);
    
    let errorMessage =
        `${process.env.EMOJI_ERROR}  The following data was entered ` +
            'incorrectly:\n';
    let errorState = false;

    if (!recipientValidation) {
        errorMessage +=
            `${process.env.EMOJI_SMALL_WHITE_DASH}User ID. It should only ` +
                'contain numbers.\n';
        errorState = true;
    }
    ...
    if (!recipientCardValidation) {
        errorMessage +=
            `${process.env.EMOJI_SMALL_WHITE_DASH}Desired card. Card ID ` +
                'format is `SCP-XXXX`.\n';
        errorState = true;
    }
    ...
    if (!recipientHolographicValidation) {
        errorMessage +=
            `${process.env.EMOJI_SMALL_WHITE_DASH}Desired holographic. ` +
                'Allowed values are Emerald/Golden/Diamond.\n';
        errorState = true;
    }

    if (!issuerCardValidation) {
        errorMessage +=
            `${process.env.EMOJI_SMALL_WHITE_DASH}Offered card. Card ID ` +
                'format is `SCP-XXXX`.\n';
        errorState = true;
    }
    ...
    if (!issuerHolographicValidation) {
        errorMessage +=
            `${process.env.EMOJI_SMALL_WHITE_DASH}Offered holographic. ` +
                'Allowed values are Emerald/Golden/Diamond.';
        errorState = true;
    }
    ...
    return {
        recipientValue,
        fixedIssuerCardValue,
        fixedIssuerHolographicValue,
        fixedRecipientCardValue,
        fixedRecipientHolographicValue,
        errorState,
        errorMessage,
    };
}

// * This function does special validations that are not covered by the previous
// * function, such as trading with yourself or trading the same card.
function validateSpecialConditions(
    userId,
    recipientValue,
    issuerCardValue,
    issuerHolographicValue,
    ...
    let errorMessage =
        `${process.env.EMOJI_ERROR}  You can't perform the following ` +
            'actions:\n';
    let errorState = false;
    
    if (userId === recipientValue) {
        errorMessage +=
            `${process.env.EMOJI_SMALL_WHITE_DASH}You can't trade with ` +
                'yourself.\n';
        errorState = true;
    }

    if (issuerCardValue === recipientCardValue &&
        issuerHolographicValue === recipientHolographicValue) {

        errorMessage +=
            `${process.env.EMOJI_SMALL_WHITE_DASH}You can't trade the same ` +
                'card.';
        errorState = true;
    }
