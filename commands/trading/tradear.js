    ...
    async execute(interaction) {
        ...
        if (snapshotUsuarioEmisor.exists) {
            ...
            interaction.awaitModalSubmit({ filter: filtro, time: tiempo }).then(async (modalInteraction) => {
                ...
                const cartaEncontradaEmisor = encontrarCarta(referenciaUsuarioEmisor, valorCartaEmisor);
                ...
                if (cartaEncontradaEmisor) {
                    ...
                    if (snapshotUsuarioReceptor.exists) {
                        const cartaEncontradaReceptor = encontrarCarta(referenciaUsuarioReceptor, valorCartaReceptor);
                        ...
                        if (cartaEncontradaReceptor) {
                            ...
                            console.log('Se realiza el tradeo');
                            modalInteraction.editReply('traka');
                        } else {
...
async function encontrarCarta(referenciaUsuario, valorCarta) {
    const referenciaCartasEmisor = database.collection('obtencion').where('usuario', '==', referenciaUsuario);
